...
// Tipagem do agregado (public.tournaments)
...
// Tipagem do “player row” (tournaments_raw)
...
const DeepDiveView: React.FC<DeepDiveViewProps> = ({ dataVersion }) => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // 1) Carrega agregado (base principal do DeepDive)
      ...
      if (aggResp.error) {
        console.error("Erro carregando tournaments:", aggResp.error.message);
        setTournamentsAgg([]);
      } else {
        setTournamentsAgg((aggResp.data ?? []) as TournamentAggRow[]);
      }

      // 2) Carrega “players” a partir do raw (amostra limitada para não travar)
      // ⚠️ Para bases gigantes, depois a gente troca isso por VIEW/RPC com DISTINCT.
      const rawResp = await supabase
        .from("tournaments_raw")
        // Essas colunas no seu schema estão com letras maiúsculas, mas sem espaços no começo.
        // Se der erro aqui, me avisa que eu ajusto para o nome exato retornado pelo PostgREST.
        .select('Rede,Jogador')
        .limit(5000);

      if (rawResp.error) {
        console.warn("Não consegui carregar players do raw (ok por enquanto):", rawResp.error.message);
        setPlayers([]);
      } else {
        ...
        for (const r of rawResp.data ?? []) {
          const rede = (r as any).Rede ?? "";
          const jogador = (r as any).Jogador ?? "";
          if (!rede || !jogador) continue;

          // id único pro checkbox
        ...
        // ordena pra ficar bom no dropdown
  ...
  // =========================
  // 2) UNIQUE LISTS (FROM DB)
  // =========================
  const uniqueRedes = useMemo(() => {
    const redes = Array.from(new Set(tournamentsAgg.map((t) => t.rede).filter(Boolean)));
  ...
  const handleKeywordChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (val.trim().length >= 1) {
      ...
      const vMatches = uniqueVelocities
        ...
        .map((v: string) => ({ value: v, type: "Velocidade" }));
  ...
  // =========================
  // 4) NETWORKS FROM PLAYERS
  // =========================
  const networksFromPlayers = useMemo(() => {
    const sel = new Set(selectedJogadores);
    const redes = players.filter((p) => sel.has(p.id)).map((p) => p.rede);
    return Array.from(new Set(redes));
  }, [players, selectedJogadores]);

  const activeNetworksForSearch = useMemo(() => {
    return Array.from(new Set([...networksFromPlayers, ...selectedRedes]));
  }, [networksFromPlayers, selectedRedes]);

  // =========================
  // 5) SUMMARIES (FROM tournaments)
  // =========================
  const summaries = useMemo(() => {
    ...
    if (activeNetworksForSearch.length > 0) {
      ...
      list = list.filter((t) => set.has(t.rede));
    }

    // Formato compatível com o resto do componente
    return list.map((t) => ({
      rede: t.rede,
      nome: t.nome,
      qtd: t.games_count,
      velocidadePredominante: t.velocidade ?? "",
    }));
  }, [tournamentsAgg, activeNetworksForSearch]);
  ...
  // =========================
  // 6) BASE DETAILED RESULTS (FROM tournaments)
  // =========================
  const baseDetailedResults = useMemo(() => {
    let list = tournamentsAgg;

    // redes selecionadas (inclui redes das chaves)
    const selectedRedesFromKeys = selectedKeys.map((k) => parseTournamentKey(k).rede).filter((r) => r && r.trim() !== "");
    const selectedRedesForService = Array.from(new Set([...(selectedRedes || []), ...selectedRedesFromKeys]));
    ...
    // se selecionou torneios específicos (selectedKeys) → filtra por tournament_key
    if (selectedKeys.length > 0) {
      const set = new Set(selectedKeys);
      list = list.filter((t) => set.has(t.tournament_key));
    } else if (activeKeywords.length > 0) {
      // senão, usa keywords para filtrar por nome/rede/velocidade
      const kws = activeKeywords.map((k) => k.toLowerCase());
      list = list.filter((t) => {
        const nome = (t.nome || "").toLowerCase();
        const rede = (t.rede || "").toLowerCase();
        const vel = (t.velocidade || "").toLowerCase();
        return kws.some((k) => nome.includes(k) || rede.includes(k) || vel.includes(k));
      });
    }

    // Converte para SelectionDetailRow (compatível com o resto do componente)
    const rows: SelectionDetailRow[] = list.map((t) => ({
      nome: t.nome,
      rede: t.rede,
      // campos esperados pelo seu filtro/tabla
      stakeMedia: Number(t.avg_stake ?? 0),
      qtd: Number(t.games_count ?? 0),
      itm: Number(t.itm_count ?? 0),
      itmPercentual: Number(t.itm_pct ?? 0),
      retornoTotal: Number(t.total_profit ?? 0),
      roiTotal: Number(t.roi_total_pct ?? 0),
      mediaParticipantes: Number(t.field_avg ?? 0),
      velocidadePredominante: (t.velocidade ?? "") as any,
      // Não existem no agregado hoje:
      horario: "00:00",
      bandeiras: "",
    })) as any;

    return rows;
  }, [tournamentsAgg, selectedKeys, activeKeywords, selectedRedes]);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-3 space-y-4">
            ...
            <div className="relative" ref={suggestionRef}>
              <input
                type="text"
                placeholder={loading ? "Carregando..." : "Ex: Bounty, PKO, Turbo..."}
          ...
          <div className="lg:col-span-6 space-y-4">
            ...
            <div className="bg-slate-950 border border-slate-800 rounded-2xl overflow-hidden focus-within:ring-2 focus-within:ring-blue-600 transition-all">
              <input
                type="text"
                placeholder={loading ? "Carregando..." : "Pesquisar por nome do torneio na base de dados..."}
